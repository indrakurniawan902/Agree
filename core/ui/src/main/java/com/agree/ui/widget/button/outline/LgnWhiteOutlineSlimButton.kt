...
import com.telkom.legion.component.button.outline.base.LgnSlimOutlineButton
import com.telkom.legion.component.utility.enums.ButtonColors

class LgnWhiteOutlineSlimButton @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : LgnSlimOutlineButton(context, attrs, defStyleAttr) {
    ...
    init {
        ...
        attrs?.let {
            val attr =
                context.obtainStyledAttributes(it, R.styleable.LgnWhiteOutlineSlimButton, 0, 0)

            runCatching {
                binding.root.cardElevation = 0f
                text = attr.getString(R.styleable.LgnWhiteOutlineSlimButton_android_text).orEmpty()
                textSize = attr.getString(R.styleable.LgnWhiteOutlineSlimButton_textSize).orEmpty()
                isLoading = attr.getBoolean(R.styleable.LgnWhiteOutlineSlimButton_isLoading, false)
                startIconDrawable =
                    attr.getDrawable(R.styleable.LgnWhiteOutlineSlimButton_startIconDrawable)
                endIconDrawable =
                    attr.getDrawable(R.styleable.LgnWhiteOutlineSlimButton_endIconDrawable)
                colorSet =
                    parseColorSet(attr.getInt(R.styleable.LgnWhiteOutlineSlimButton_colorSet, 0))
                isEnable =
                    attr.getBoolean(R.styleable.LgnWhiteOutlineSlimButton_android_enabled, true)
