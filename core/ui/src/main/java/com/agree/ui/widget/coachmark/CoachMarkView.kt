...
import com.telkom.legion.component.button.fill.LgnPrimaryLargeButton

class CoachMarkView @JvmOverloads constructor(
    context: Context,
    builder: CoachMark.Builder? = null
) : FrameLayout(context) {
    ...
    private fun onInitPaint(builder: CoachMark.Builder?) {
        ...
        paintArrow?.color = builder.backgroundColor ?: 0

        setBackgroundColor(builder.shadowColor)
    }
    ...
    private fun onInitViews(context: Context, builder: CoachMark.Builder?) {
        ...
        if (layout is ViewGroup) {
            viewGroup = layout
            val root = viewGroup?.findViewById<ViewGroup>(R.id.containerRoot)
            textViewTitle = viewGroup?.findViewById(R.id.tvCoachmarkTitle)
            textDescription = viewGroup?.findViewById(R.id.tvCoachmarkDescription)
            buttonSkip = viewGroup?.findViewById(R.id.btn_coachmark_skip)
            buttonPrevious = viewGroup?.findViewById(R.id.btnCoachmarkPrevious)
            buttonNext = viewGroup?.findViewById(R.id.btnCoachmarkNext)
            textIndicator = viewGroup?.findViewById(R.id.tvCoachmarkIndicator)

            if (root is CardView) {
                root.setCardBackgroundColor(builder.backgroundColor ?: 0)
            } else {
                root?.backgroundColor(builder.backgroundColor ?: 0)
            }

            /**
             * Set text color
             */
            textViewTitle?.setTextColor(builder.textTitleColor ?: 0)
            textDescription?.setTextColor(builder.textDescriptionColor ?: 0)

            buttonSkip?.text = builder.buttonSkipText.orEmpty()
            buttonSkip?.setOnClickListener {
                if (stepPositionListener != null) {
                    if (isStart && builder.isSkip) stepPositionListener?.onComplete()
                }
            }
            buttonSkip?.colorSet = builder.buttonSkipColor

            buttonPrevious?.text = builder.buttonPreviousText.orEmpty()
            buttonPrevious?.setOnClickListener {
                if (stepPositionListener != null) {
                    stepPositionListener?.onPrevious()
                }
            }
            buttonPrevious?.colorSet = builder.buttonPreviousColor

            buttonNext?.text = builder.buttonNextText.orEmpty()
            buttonNext?.setOnClickListener { onNextView() }
            buttonNext?.colorSet = builder.buttonNextColor
    ...
    fun show(
        ...
        if (isLast) {
            buttonNext?.text = builder?.buttonFinishText.orEmpty()
    ...
    private var buttonSkip: LgnPrimaryLargeButton? = null
    private var buttonNext: LgnPrimaryLargeButton? = null
    private var buttonPrevious: LgnPrimaryLargeButton? = null
